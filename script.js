...
function writeRow(sheetName, rowData) {
  return new Promise((resolve) => {
    const cfg = getConfig();
    if (!cfg.appsScriptUrl) { console.warn('No Apps Script URL'); resolve(false); return; }
    ...
    console.log('[AK Write] Sending to sheet:', sheetName, rowData);
    console.log('[AK Write] URL:', url);

    const script = document.createElement('script');
    const timer  = setTimeout(() => {
      console.warn('[AK Write] Timeout — write may still have succeeded');
    ...
    window[callbackName] = function(response) {
      console.log('[AK Write] Response:', response);
      cleanup(); resolve(true);
    };

    script.onerror = function(e) {
      console.warn('[AK Write] Script error:', e);
      cleanup(); resolve(true);
    };
...
// ─── DATE HELPERS ─────────────────────────────────────────────
function today() {
  // Always returns DD/MM/YYYY — consistent for both writing and comparing
  const d = new Date();
...
// Normalize any date string to DD/MM/YYYY for safe comparison
...
function extractMember(row, fallbackId) {
  // Log ALL keys and values so we can see exactly what Google Sheets sends
  console.log('=== RAW ROW FROM SHEET ===');
  Object.keys(row).forEach(k => console.log(`  KEY: "${k}" => VALUE: "${row[k]}"`));
  console.log('=========================');

  const expiry = row['Package Validity'] || '';
  const parsed = parseDate(expiry);
  console.log('Package Validity raw:', JSON.stringify(expiry));
  console.log('parseDate result:', parsed);
...
async function recordAttendance(member, status){
  const row = [today(), nowTime(), String(member.memberID), member.name, status];
  console.log('[AK Attendance] Writing row:', row);
  const result = await writeRow(SHEETS.ATTENDANCE_SHEET, row);
  console.log('[AK Attendance] Write result:', result);
  return result;
}
